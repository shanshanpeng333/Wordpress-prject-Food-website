...
function hoot_theme_customizer_options() {
	...
	$sections[ $section ] = array(
		'title'       => __( 'Demo / Support', 'creattica' ),
		'priority'    => '2',
	);
	...
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 ...
				 __( "Demo the theme features and options with sample content.", 'creattica') .
				 '</span></a>';
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 'https://wphoot.com/support/' .
				 '" target="_blank"><span class="hoot-cust-link-head">' .
				 '<i class="far fa-life-ring"></i> ' .
				 __( "Documentation / Support", 'creattica') . 
				 '</span><span class="hoot-cust-link-desc">' .
				 __( "Get theme related support for both free and premium users.", 'creattica') .
				 '</span></a>';
	$lcontent .= '<a class="hoot-cust-link" href="' .
				 'https://wordpress.org/support/theme/creattica/reviews/?filter=5#new-post' .
				 '" target="_blank"><span class="hoot-cust-link-head">' .
				 '5 <i class="fas fa-star"></i> ' .
				 __( "Rate Us", 'creattica') . 
				 '</span><span class="hoot-cust-link-desc">' .
				 __( "If you are happy with the theme, give us a 5 star rating on wordpress.org", 'creattica') .
				 '</span></a>';

	$settings['linksection'] = array(
		// 'label'       => __( 'Misc Links', 'creattica' ),
		'section'     => $section,
		'type'        => 'content',
	...
	$settings["frontpage_sectionbg_slider_img"] = array(
		...
		'options'     => array(
			'description' => array(
				'label'       => '',
				'type'        => 'content',
				'content'     => '<span class="hoot-module-bg-title">' . __('Slider Background', 'creattica') . '</span>',
			),
			'type' => array(
	...
	$settings['wt_html_slider_min_height'] = array(
		...
		'default'     => 375,
		'description' => __('Leave empty to let the slider height adjust automatically.', 'creattica'),
		'input_attrs' => array(
	...
	if ( current_theme_supports( 'woocommerce' ) ) :
		...
		$sections[ $section ] = array(
			'title'       => __( 'WooCommerce (Creattica)', 'creattica' ),
		...
		$settings['wooshop_products'] = array(
			...
			'section'     => $section,
			'type'        => 'select',
			'priority'    => '975', // Non static options must have a priority
			'choices'     => $wooproducts,
			'default'     => '12',
...
function hoot_theme_customizer_enqueue_scripts() {
	// Enqueue Styles
	$style_uri = ( function_exists( 'hybridextend_locate_style' ) ) ? hybridextend_locate_style( HYBRIDEXTEND_INCURI . 'admin/css/customize' ) : HYBRIDEXTEND_INCURI . 'admin/css/customize.css';
	wp_enqueue_style( 'hoot-theme-customize-styles', $style_uri, array(),  HYBRIDEXTEND_VERSION );
	// Enqueue Scripts
	$script_uri = ( function_exists( 'hybridextend_locate_script' ) ) ? hybridextend_locate_script( HYBRIDEXTEND_INCURI . 'admin/js/customize' ) : HYBRIDEXTEND_INCURI . 'admin/js/customize.js';
	wp_enqueue_script( 'hoot-theme-customize-script', $script_uri, array( 'jquery', 'wp-color-picker', 'customize-controls', 'hybridextend-customize-script' ), HYBRIDEXTEND_VERSION, true );
}
